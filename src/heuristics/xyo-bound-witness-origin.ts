...
export class XyoBoundWitnessOriginGetter {
  public static getOriginInformation(boundWitness: XyoBoundWitness): IXyoBoundWitnessOrigin[]  {
    ...
    for (const fetterHeuristics of allHeuristics) {
      origins.push(XyoBoundWitnessOriginGetter.getOriginInformationFromFetter(fetterHeuristics))
    }

    return origins
  }

  private static getOriginInformationFromFetter(fetterHeuristics: XyoStructure[]): IXyoBoundWitnessOrigin  {
    ...
    for (const heuristic of fetterHeuristics) {
      ...
      if (heuristic.getSchema().id === XyoObjectSchema.INDEX.id) {
        index = indexResolver.resolve(heuristic.getAll().getContentsCopy()).value
      }
    }
