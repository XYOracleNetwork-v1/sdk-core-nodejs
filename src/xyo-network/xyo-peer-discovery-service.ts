 ...
 * @Date:   Thursday, 1st November 2018 1:06:59 pm
...
export class XyoPeerDiscoveryService extends XyoBase {
  ...
  private me?: IPeerDescriptionWithPeers;

  private readonly queue: IPeerCandidate[] = [];
  ...
  private onPeerFound: ((peers: IPeerDescription) => boolean) | undefined;
  ...
  constructor(
    private readonly peerCandidates: IPeerCandidate[],
  ...
  public addPeerCandidate(candidate: IPeerCandidate) {
    return this.addCandidateToQueue(candidate.ip, candidate.port);
  }

  public updatePeerDescription(peerDescription?: IPeerDescriptionWithPeers) {
  ...
  public findPeers(onPeerFound: (peers: IPeerDescription) => boolean): () => void {
  ...
  private processQueue(queueInterval: number = this.MIN_QUEUE_WAIT) {
    this.queueProcessTimeout = setTimeout(async () => {
      ...
      if (this.queue.length > 0) {
        const item = this.queue.pop() as IPeerCandidate;
  ...
  private potentialPeerIsNotBlackListed(potentialPeer: IPeerDescription) {
  ...
  private getAboutForPeer(
    hostname: string,
    port: number,
    me?: IPeerDescriptionWithPeers
  ): Promise<IPeerDescriptionWithPeers | undefined> {
          ...
          res.on("end", () => {
            ...
            try {
              ...
              if (
                ...
                const result: IPeerDescriptionWithPeers = {
    ...
    }) as Promise<IPeerDescriptionWithPeers | undefined>;

  }

  private aboutMeSerializer(me: IPeerDescription | IPeerDescriptionWithPeers, withPeers: boolean) {
    let peersValue = '[]';
    if (withPeers && this.isPeerDescriptionWithPeers(me) && me.peers.length) {
  ...
  private isPeerDescriptionWithPeers(
    peerDescription: IPeerDescription | IPeerDescriptionWithPeers
  ): peerDescription is IPeerDescriptionWithPeers {
    return (peerDescription as IPeerDescriptionWithPeers).peers instanceof Array;
  }
}

export interface IPeerCandidate {
  ip: string;
  port: number;
}

export interface IPeerDescription {
  name: string;
  version: string;
  ip: string;
  graphqlPort: number;
...
export interface IPeerDescriptionWithPeers extends IPeerDescription {
  peers: IPeerDescription[];
}
